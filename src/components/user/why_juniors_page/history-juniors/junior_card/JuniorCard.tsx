    ...
    <div className="flex items-center justify-center">
      <div className="bg-secondaryGray rounded-2xl px-3 sm:px-5 py-6 md:px-6 w-full max-w-[540px]">
        <div className="relative w-full h-[346px] sm:h-[430px] sm+:h-[450px] md:h-[470px] rounded-md overflow-hidden p-4">
          { !isOpen &&  <Image src={data.img} fill alt={data.name} className="brightness-[60%]" sizes="100%" objectFit="cover" objectPosition="top"/>}

          <div className={clsx("absolute z-10 bottom-0 left-0 text-white font-normal md:font-semibold px-1 p-1 md:px-6 md:pt-6 flex flex-col h-full md:h-fit overflow-y-auto", 
          ...
          onClick={()=>{isOpen && setIsopen(false)}}>
            ...
            <p className="text-sm md:mb-5 -order-1 md:order-none">{formatDate(data.date, locale)}</p>
            ...
            {!isOpen && <button type="button" 
                className="min-w-[230px] sm:max-w-[240px] flex items-center justify-center px-1 py-4 main-gradient text-black rounded-md mb-1 md:mb-6 text-xl" onClick={()=>{setIsopen(true)}}>
                  Докладніше
              </button>
            }
